...
def get_span(label, type):
    ...
    for k in range(len(label)):
        if label[k] == 'I' and flag == False:
            flag = True
            st = k
        if label[k] != 'I' and flag == True:
            flag = False
            en = k
            if type:
...
def evaluation_my(gold,pred,type):
    right = 0
    predicted = 0
    total_en = 0
    #fout = open('log.valid', 'w')
    for i in range(len(gold)):
        gold_label = gold[i].split()
        pred_label = pred[i]
        gold_span = get_span(gold_label, type)
        pred_span = get_span(pred_label, type)
        #fout.write('{}\n{}'.format(gold_span, pred_span))
        total_en += len(gold_span)
        predicted += len(pred_span)
        for item in pred_span:
