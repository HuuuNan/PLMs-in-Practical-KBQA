...
def _extract_disamb_feature_from_problem(args, tokenizer, problem):
    ...
    for c in problem.candidates:
        ...
        # else:
        #     print('WANING VALID LABEL', c.id, '|', c.label, '|', c.facc_label)
        if args.do_lower_case:
...
def extract_disamb_features_from_examples(args, tokenizer, instances, do_predict=False):
    ...
    all_empty_ents = 0
    w_full_ents = 0
    print("instances length: %d" %len(instances))
    ...
    hints = 'VALID: {}, ACC: {:.2f}'.format(len(valid_disamb_problems), baseline_acc / len(valid_disamb_problems))
    print("r_full_ents: %d, w_full_ents: %d, r_empty_ents: %d, w_empty_ents:%d, all_empty_ents: %d" %(len(valid_disamb_problems), w_full_ents, r_empty_ents, w_empty_ents, all_empty_ents))
    features = []
